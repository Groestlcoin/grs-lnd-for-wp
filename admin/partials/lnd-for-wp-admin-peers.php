...
<div class="lnd-wp-status">
	...
	if( $lnd_peers ){
		...
		if( $dc_peer_confirm ){
			...
			<form class="lnd-peer-form" method="post" action="?page=<?php echo esc_html( $_REQUEST['page'] ); ?>&f=peers">
				...
				<input type="hidden" name="lnd-post-nonce" value="<?php echo wp_create_nonce( 'lnd-disconnect-peer' ); ?>" />
		...
		<?php }else if(!isset( $_REQUEST['lnd-add-peer'] )){ ?>
			...
			<?php foreach( $lnd_peers as $lnd_peer ){ ?>
				...
				<div class="lnd-peer-contain">
					...
					<form class="lnd-in-line" method="post" action="?page=<?php echo esc_html( $_REQUEST['page'] ); ?>&f=channels">
					...
					<form class="lnd-in-line" method="post" action="?page=<?php echo esc_html( $_REQUEST['page'] ); ?>&f=peers">
						<input type="hidden" name="lnd-disconnect-peer" value="Y" />
	...
	<?php if(!isset( $_REQUEST['lnd-add-peer'] ) && !$dc_peer_confirm ){ ?>
		...
		<form class="lnd-peer-form" method="post" action="?page=<?php echo esc_html( $_REQUEST['page'] ); ?>&f=peers">
			...
			<input type="hidden" name="lnd-post-nonce" value="<?php echo wp_create_nonce( 'lnd-add-peer' ); ?>" />
	...
	<?php }else if(isset( $_REQUEST['lnd-add-peer'] ) && $_REQUEST['lnd-add-peer'] == "Y") { ?>
			...
			<form method="post" action="?page=<?php echo esc_html( $_REQUEST['page'] ); ?>&f=peers">
